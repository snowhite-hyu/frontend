        ...
        <div className="flex flex-inline w-full h-full items-start">
            <div className="flex flex-col w-full mt-[10%] mr-[10%] ml-auto">
                <div className="w-full max-w-[500px] min-w-[300px] h-[339px] rounded-[21.87px] bg-[#ffffff] mr-0 ml-auto pt-[26px] pl-[34px] pr-[26px]">
                    {rooms.map((room) =>(
                        <div key={room} className="w-full flex flex-inline mb-[10px] items-center ">
                            <p className="text-[#000000] text-[25px] ml-[0%] mr-auto">{room}</p>
                            <Button variant={"saboteurCheck"} className="h-fit mr-[0%] ml-auto text-[21px]" onClick={() => navigate("/waiting")}>
                                참가
                            </Button>
                        </div>
                    ))}
                </div>
                <Button variant={"sabotuer"} className="w-fit h-fit mt-[3%] mr-[0%] ml-auto" type="submit" onClick={() => navigate("/create-room")}>
                    생성
                </Button>
            </div>
        </div>
